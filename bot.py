...
add_group_handler = ConversationHandler(entry_points=[CommandHandler('add_group', add_group)],
                                        states={
                                            1: [MessageHandler(Filters.text, add_group)]
                                        },
...
start_handler = ConversationHandler(entry_points=[MessageHandler(Filters.text, identification)],
                                    states={
                                        0: [CommandHandler('start', start), CommandHandler('help', help_callback),
                                            add_group_handler, show_groups_handler, delete_groups_handler,
                                            add_chat_handler, add_user_handler, delete_chat_handler, error_handler]
                                    },
                                    fallbacks=[MessageHandler(Filters.text, identification)])

if __name__ == '__main__':
    updater = Updater(token='bot_token', use_context=True, workers=20)
