...
class DeleteChat:
    ...
    def delete_chat_button(update: Update, context: CallbackContext):
        ...
        with open('groups.json', 'r') as file:
            DeleteChat.choice = json.loads(file.read())[choice]['chats']
    ...
    def ending_delete_chat(update: Update, context: CallbackContext):
        ...
        if data == 'exit':
            ...
            page = int(data.split('|||')[1])
            all_chats = DeleteChat.choice
            reply_markup = Markup(all_chats, page)
        ...
        with open('groups.json', 'r') as file:
            all_groups = json.loads(file.read())
        all_groups[DeleteChat.choice_group]['chats'].remove(choice)
        with open('groups.json', 'w') as file:
            file.write(json.dumps(all_groups))
